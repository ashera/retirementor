...
export function retirementYearIncome(row: YearRow): RetirementYearIncome {
  const zero: RetirementYearIncome = {
    pension: 0, netRent: 0, partnerSalary: 0, partTimeWork: 0,
  ...
  // work). The ATO minimum can push super out beyond this — that surplus is
  // reinvested, not spent, so it isn't spendable income this year.
  const need = Math.max(0, row.spending - pension - netRent - partnerSalary - partTimeWork);
  const superReinvested = Math.max(0, drawnSuper - need);
  const fromSuper = drawnSuper - superReinvested;

  const total = pension + netRent + fromSuper + fromOutside + partnerSalary + partTimeWork;
  return { pension, netRent, partnerSalary, partTimeWork, fromSuper, fromOutside, superReinvested, need, total };
}
