...
export interface RetirementPlan {
  ...
  recontribute?: { perYear: number; fromAge: number; untilAge: number; source?: "savings" | "super" }; // recontribution: after-tax (non-concessional) top-up of super, each year from fromAge to untilAge (a one-off when they're equal), age ≤75, within the NCC cap. source "savings" (DEFAULT) tops super up from OUTSIDE savings (grows super, shelters money in the tax-free environment). source "super" is the cash-out-and-recontribute death-tax play: person 0 withdraws from their OWN super (tax-free over 60) and immediately re-contributes it as non-concessional — a balance-neutral round trip that converts perYear×(taxable proportion) of super from the TAXABLE component to the TAX-FREE component, cutting the [[super death-benefit tax]] to a non-dependant. See config.superDeathBenefit + plan.superBeneficiary.
...
export interface YearBreakdown {
  ...
  radDrawn?: number;
  radHeld?: number;
  agedCareRadRetention?: number; // RAD retention kept by the provider this year (2%/yr of the deposit, up to 5 years) — reduces the refundable balance held to the estate
