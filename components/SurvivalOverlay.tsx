...
function RbdTooltip({ active, payload, ages }: { active?: boolean; payload?: { payload: RbdPoint }[]; ages: AgeGapInfo | null }) {
  if (!active || !payload?.length) return null;
  const p = payload[0].payload;
  return (
    <div className="rounded-lg border border-line bg-panel px-3 py-2 text-xs shadow-lg">
      <div className="mb-1 font-semibold text-white">{ages ? dualAgeLabel(ages, p.age) : `Age ${p.age}`}</div>
      <div className="flex items-center justify-between gap-4 text-emerald-300"><span>Alive · money lasting</span><span className="tabular-nums">{pct(p.solvent)}</span></div>
      <div className="flex items-center justify-between gap-4 text-red-300"><span>Alive · run out</span><span className="tabular-nums">{pct(p.broke)}</span></div>
      ...
      <div className="h-64 w-full">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 8, right: 8, bottom: 4, left: -8 }}>
            ...
            <Tooltip content={<RbdTooltip ages={ages} />} />
            ...
            {lens.medianAgeAtDeath != null && (
              <ReferenceLine x={lens.medianAgeAtDeath} stroke="#8b97ad" strokeDasharray="2 3" strokeOpacity={0.7}
                label={{ value: "50% still alive", position: "insideTopRight", fill: "#8b97ad", fontSize: 10 }} />
            )}
          </AreaChart>
        </ResponsiveContainer>
      </div>
      <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted">
        <span className="flex items-center gap-1.5"><span className="inline-block h-2.5 w-2.5 rounded-sm" style={{ background: "#10b981" }} />Alive · money lasting</span>
        <span className="flex items-center gap-1.5"><span className="inline-block h-2.5 w-2.5 rounded-sm" style={{ background: "#ef4444" }} />Alive · run out</span>
