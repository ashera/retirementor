        ...
        {(strategies.length > 0 || events.length > 0) && (
          <div className="break-before-page">
            ...
            <Section title="Net worth over time">
              ...
              <div className="rounded-lg border border-slate-200 bg-slate-50 p-2">
                <RetirementChart result={result} showHome bands={bands} animate={false} height={300} wageInflationPct={wageInfl} cpiPct={plan.inflation} ages={ageGapInfo(plan)} lifeEvents={plan.lifeEvents} agedCare={plan.agedCare?.enabled ? { entryAge: plan.agedCare.entryAge, durationYears: plan.agedCare.durationYears } : null} plan={plan} />
              </div>
            </Section>
          </div>
        )}
